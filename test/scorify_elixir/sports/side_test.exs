defmodule ScorifyElixir.Sports.SideTest do
  ...
  setup do
    ...
    create_side_with_league_season! = fn ->
      ...
      league_season =
        ...
        |> LeagueSeason.changeset(%{start_date: "2018-08-15", end_date: "2019-05-15"})
        |> Repo.insert!()
  ...
  test "changeset with name that already exists for the same sport", %{
    ...
    assert {:error, _} =
             %Side{} |> Side.changeset(%{@valid_attrs | sport_id: sport.id}) |> Repo.insert()

    assert {:ok, _} =
             %Side{} |> Side.changeset(%{@valid_attrs | sport_id: other_sport.id})
             |> Repo.insert()
  end
  ...
  test "has many :league_seasons", %{
    create_side_with_league_season!: create_side_with_league_season!
  } do
    {side, %LeagueSeason{id: league_season_id}} = create_side_with_league_season!.()

    assert [%LeagueSeason{id: ^league_season_id} | []] =
             side |> assoc(:league_seasons) |> Repo.all()
  end
